				...
				case -3:
					{
						...
						player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(2 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
						...
						if (NPC.ai[1] == deathAnimationTime)
						{
							...
							player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(36 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
				...
				case 0:
					{
						...
						if (NPC.ai[1] < part1Time)
						{
							...
							player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(1 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
						...
						NPC.ai[1]++;
						if (NPC.ai[1] == part1Time + part2Time) gotoNextAttack = true;
					}
					break;
				#endregion
							...
							else
							{
								...
								if (inPhase2 && (attackProgress - (attackSetupTime + attackFreezeTime)) % 2 == 0 && attackProgress != attackSetupTime + attackFreezeTime) //exclude the first projectile because it looks weird
								{
									...
									Projectile.NewProjectile(
										...
										NPC.Center + new Vector2(280, 0).RotatedBy(NPC.rotation), 
										new Vector2(projSpeed, 0).RotatedBy(NPC.rotation), 
										ProjectileType<ConvectiveWandererAcceleratingShot>(), 
										12, 
										2f, 
										Main.myPlayer, 
										ai0: 2f, 
										ai1: inPhase2 ? 0.5f : 0f
									);
								}
				...
				case 2:
					{
						...
						if (NPC.ai[1] >= totalAttackTime * attackRepetitions + totalAttackSetupTime)
						{
							...
							if (timeLeft > 60)
							{
								...
								if (NPC.ai[1] == totalAttackTime * attackRepetitions + totalAttackSetupTime)
								{
									...
									player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(40 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
						...
						else if (attackProgress < totalAttackSetupTime + totalAttackDashTime)
						{
							...
							player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(20 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
				...
				case 5:
					{
						...
						else if (NPC.ai[1] < setupTime)
						{
							...
							else if (NPC.ai[1] < setupStartTime + setupMidTime + setupEndTime)
							{
								...
								player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(3 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
								player.GetModPlayer<PolaritiesPlayer>().screenshakeTimer = 30;
							}
							else
							{
				...
				case 6:
					{
						...
						else if (attackProgress < setupTime + attackTime)
						{
							...
							if (attackProgress == setupTime)
							{
								Projectile.NewProjectile(
									NPC.GetSource_FromAI(), 
									NPC.Center + new Vector2(80, 0).RotatedBy(NPC.rotation), 
									new Vector2(1, 0).RotatedBy(NPC.rotation), 
									ProjectileType<ConvectiveWandererFlamethrower>(), 
									16, 
									2f, 
									Main.myPlayer, 
									ai0: NPC.whoAmI, 
							...
							if ((attackProgress - setupTime) % period == 0 && attackProgress > setupTime)
							{
								...
								for (int i = 0; i < numProjectiles; i++)
								{
									Projectile.NewProjectile(
										NPC.GetSource_FromAI(), 
										NPC.Center + new Vector2(112, 0).RotatedBy(NPC.rotation), 
										new Vector2(4, 0).RotatedBy(NPC.rotation + i * MathHelper.TwoPi / numProjectiles + MathHelper.TwoPi / 8f), 
										ProjectileType<ConvectiveWandererAcceleratingShot>(), 
										12, 
										2f,
										Main.myPlayer, 
				...
				case 7:
					{
						...
						else if (attackProgress < mainAttackTime)
						{
							...
							if (timeLeft == 0)
							{
								NPC.ai[3] = Projectile.NewProjectile(
									NPC.GetSource_FromAI(), 
									NPC.Center + new Vector2(240, 0).RotatedBy(NPC.rotation), 
									NPC.velocity, 
									ProjectileType<ConvectiveWandererChargedShot>(), 
									16, 
									2f, 
									Main.myPlayer, 
									ai0: NPC.whoAmI, 
				...
				case 8:
					{
						...
						else if (attackProgress < dashSetupTime + dashStartTime)
						{
							...
							//player.GetModPlayer<PolaritiesPlayer>().AddScreenShake(0.25f * NPC.velocity.Length() / Math.Max(NPC.Distance(player.Center) / 200f, 1f), 10);
							player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(26 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
							player.GetModPlayer<PolaritiesPlayer>().screenshakeTimer = 120;
						}
						else
						{
							...
							if (timeLeft == 1)
							{
								...
								for (int i = 0; i < numProjectiles; i++)
								{
									Projectile.NewProjectile(
										NPC.GetSource_FromAI(), 
										NPC.Center + new Vector2(112, 0).RotatedBy(NPC.rotation), 
										new Vector2((inPhase2 && i % 2 == 1) ? 6 : 8, 0).RotatedBy(i * MathHelper.TwoPi / numProjectiles + NPC.rotation), 
										ProjectileType<ConvectiveWandererAcceleratingShot>(), 
										12, 
										2f, 
										Main.myPlayer, 
								...
								{
									ConvectiveWandererVortexParticle particle = Particle.NewParticle<ConvectiveWandererVortexParticle>(
										NPC.Center + new Vector2(112, 0).RotatedBy(NPC.rotation), 
										new Vector2(Main.rand.NextFloat(12f, 40f), 0).RotatedByRandom(MathHelper.TwoPi), 
										0f, 
										0f, 
				...
				case 9:
					{
						...
						else
						{
							if (NPC.ai[1] == setupTime + attackTime)
							{
								...
								//player.GetModPlayer<PolaritiesPlayer>().AddScreenShake(40, 120);
								Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(40 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
...
private void Main_UpdateMenu(MonoMod.Cil.ILContext il)
	...
	c.EmitDelegate<Action>(() =>
	{
		...
		public override void AI()
		{
				...
				if (Projectile.timeLeft % 60 == 0 && Projectile.timeLeft <= 600)
				{
					...
					for (int i = 0; i < numProjectiles; i++)
					{
						Projectile.NewProjectile(
							...
							Projectile.Center, 
							new Vector2(4, 0).RotatedBy(i * MathHelper.TwoPi / numProjectiles + randRotation), 
							ProjectileType<ConvectiveWandererAcceleratingShot>(), 
							12, 
							2f, 
							Main.myPlayer, 
							ai0: Projectile.localAI[0], 
							ai1: Projectile.ai[1]
						);
					}
			...
			Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(26 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
		...
		public override void OnKill(int timeLeft)
		{
			...
			for (int i = 0; i < numProjectilesPerRing; i++)
			{
				for (int j = 0; j < numProjectilesPerAngle; j++)
				{
					...
					Projectile.NewProjectile(
						Projectile.GetSource_FromAI(), 
						Projectile.Center, 
						new Vector2(8 * speedMult, 0).RotatedBy(j * MathHelper.Pi + i * MathHelper.TwoPi / numProjectilesPerRing + randRotation), 
						ProjectileType<ConvectiveWandererAcceleratingShot>(), 
						12, 
						2f, 
						Main.myPlayer, 
						ai0: 4.5f + 0.5f * rotationAmount * Projectile.spriteDirection, 
						ai1: Projectile.ai[1]
					);
				}
			}
			...
			for (int i = 0; i < numPulses; i++)
			{
				...
				ConvectiveWandererExplosionPulseParticle particle = Particle.NewParticle<ConvectiveWandererExplosionPulseParticle>(
					Projectile.Center, 
					Vector2.Zero, 
					0f, 
					0f, 
					Scale: Projectile.scale * pulseScale * 2f, 
					Color: drawColor * (1.6f * pulseScale * (1 - pulseScale))
				);
				ParticleLayer.AfterLiquidsAdditive.Add(particle);
			}

			//Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().AddScreenShake(48, 60);
			Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(48 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
		...
		public override void AI()
		{
			if (Projectile.timeLeft == 60)
			{
				...
				if (Projectile.ai[0] == 0)
				{
					...
					Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(10 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
					Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeTimer = 60;
	...
	public class ConvectiveWandererFlamethrower : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (Projectile.timeLeft >= 10)
			{
				...
				Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(20 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
				Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeTimer = 60;
			}
		}
				...
				if (Vector2.Distance(Projectile.Center, Main.LocalPlayer.Center + Main.LocalPlayer.velocity * 15f) > 900 && Vector2.Dot(Projectile.velocity, Main.LocalPlayer.Center - Projectile.Center) < 0)
				{
					...
					for (int i = 0; i < numProjectiles; i++)
					{
						Projectile.NewProjectile(
							...
							Projectile.Center, 
							new Vector2(8, 0).RotatedBy(i * MathHelper.TwoPi / numProjectiles + rotationOffset), 
							ProjectileType<ConvectiveWandererAcceleratingShot>(), 
							12, 
							2f, 
							Projectile.owner, 
					...
					if (Projectile.ai[1] != 0)
					{
						for (int i = 0; i < numProjectiles; i++)
						{
							Projectile.NewProjectile(
								Projectile.GetSource_Death(), 
								Projectile.Center, 
								new Vector2(6, 0).RotatedBy(i * MathHelper.TwoPi / numProjectiles + MathHelper.Pi / numProjectiles + rotationOffset), 
								ProjectileType<ConvectiveWandererAcceleratingShot>(), 
								12, 
								2f, 
								Projectile.owner, 
								ai0: 1, ai1: Projectile.ai[1]
							);
						}
					}

					//Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().AddScreenShake(36, 30);
					Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = (int)(36 * ModContent.GetInstance<Global.ClientConfigurations>().CW_SCREENSHAKE_MULTIPLIER);
					Main.LocalPlayer.GetModPlayer<PolaritiesPlayer>().screenshakeTimer = 60;

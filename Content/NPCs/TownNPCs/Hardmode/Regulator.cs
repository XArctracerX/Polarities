...
namespace Polarities.Content.NPCs.TownNPCs.Hardmode
{
    [AutoloadHead]
    //TODO: Better party drawing
    public class Regulator : ModNPC
    {
        ...
        private void NPC_checkDead(ILContext il)
        {
            ...
            c.EmitDelegate<Func<int>>(() => { return NPCType<Regulator>(); });
        ...
        public override void SetStaticDefaults()
        {
            ...
            NPC.Happiness
                .SetBiomeAffection<Biomes.Fractal.FractalOceanBiome>(AffectionLevel.Like)
                .SetBiomeAffection<Biomes.Fractal.FractalSkyBiome>(AffectionLevel.Like)
                .SetBiomeAffection<Biomes.Fractal.FractalWastesBiome>(AffectionLevel.Dislike)
        ...
        private void DrawLoop(List<Vector2> list, float stroke, params Color[] colors)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                Vector2 scale = new Vector2(stroke, (diff.Length() + 2) / frame.Height);
        ...
        {
            List<Vector2> auraPositions = new List<Vector2>();
            for (int i = 0; i < 128; i++)
            {
                float angle = MathHelper.Pi * i / 64f;
                auraPositions.Add(new Vector2(300, 0).RotatedBy(angle));
            }

            bool playerInsideAura = Main.LocalPlayer.Distance(NPC.position) < 300;
            float width = 1;
            if (!playerInsideAura) width = 0.5f;

            DrawLoop(auraPositions, width, new Color(246, 156, 60), new Color(215, 120, 52));
            return true;
        }

        //Doesnt work but should fix every issue
        /*

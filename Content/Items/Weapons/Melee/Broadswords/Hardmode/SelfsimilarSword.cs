...
namespace Polarities.Content.Items.Weapons.Melee.Broadswords.Hardmode
{
	...
	public class SelfsimilarSwordShot : ModProjectile
	{
		...
		private float bladeOffset = 50f; // Here in Terraria we do not hold swords by their blades
		
		private static Asset<Texture2D> bladeTex;
		
		public override void Load(){
			bladeTex = ModContent.Request<Texture2D>("Polarities/Content/Items/Weapons/Melee/Broadswords/Hardmode/SelfsimilarSwordBlade");
		}
		...
		public override void AI()
		{
			...
			if(mode==1 || mode==2){
				...
				Projectile.position.X += 150f*dir*smooth(progress, 0f, pace);
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) { // Treat as if Projectile.penetrate=1;
			...
			if(mode==1){
				...
				List<Vector2> positions = new List<Vector2>{
					new Vector2(target.position.X+diff, target.position.Y), // This is stupid code
					new Vector2(target.position.X-diff, target.position.Y)
				};
				...
				for(int i=0; i<2; i++){
					Projectile proj = Projectile.NewProjectileDirect(
						...
						positions[i], 
						target.velocity, 
						ProjectileType<SelfsimilarSwordSecondary>(), 
						Projectile.damage,
		...
		public override void ModifyDamageHitbox (ref Rectangle hitbox) {
			if(mode>0){
				...
				Vector2 point = new Vector2(bladeOffset*(float)Math.Sin(t),bladeOffset*(float)Math.Cos(t)-20f);
				hitbox.X += (int)point.X;
				hitbox.Y += (int)point.Y;
				hitbox.Width=bladeTex.Value.Width;
				hitbox.Height=bladeTex.Value.Height;
			}
		}
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			for (int k = Projectile.oldPos.Length - 1; k > 0; k--) {
				...
				Main.EntitySpriteDraw(texture, drawPos, null, color, Projectile.rotation, drawOrigin, Projectile.scale*1.1f, SpriteEffects.None, 0);
			}
			...
			if(mode>0){
				...
				if(Projectile.spriteDirection==1){t*=-1f;}
				Vector2 point = new Vector2(bladeOffset*(float)Math.Sin(t), bladeOffset*(float)Math.Cos(t));
				Vector2 drawPos = (Projectile.position - Main.screenPosition + point) + bladeOrigin + new Vector2(0f, Projectile.gfxOffY-20f);
	...
	public class SelfsimilarSwordSecondary : ModProjectile
	{
		private int mode = -1; // -1 = initialise, 0 = inactive, 1 = attacking, 2 = fading
		private int timer = 90;
		private float time= 0f;
		private float timeDamage = 15f;
		private float dir; // Which direction to slash in
		private float side; // Which side of the NPC to start at
		private NPC t; // Target NPC
		private float bladeOffset = 50f; // Here in Terraria we do not hold swords by their blades
		
		//public void setNPC(NPC target){
		//	t = target;
		//}
		...
		public override void AI()
		{
			...
			if(mode==-1){
				mode=0;
				t=Main.npc[(int)Projectile.ai[2]];
				player=t;
				dir =-(Projectile.position.X-player.position.X)/Math.Abs(Projectile.position.X-player.position.X);
				side=(Projectile.position.X-player.position.X)/Math.Abs(Projectile.position.X-player.position.X);
				Projectile.spriteDirection = (int)-dir;
			}
			
			Projectile.position.X = player.position.X + side*90f;
			Projectile.position.Y = player.position.Y;

...
namespace Polarities.Content.Items.Weapons.Melee.Boomerangs.Hardmode
{
	public class SunDisc : ModItem
	{
		...
		public override void SetDefaults() {
			...
			Item.autoReuse = true;
			Item.shoot = ProjectileType<Solarang>();
			Item.shootSpeed = 20f;
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
			if (player.altFunctionUse == 2)
			{
				foreach (Projectile projectile in Main.projectile)
				{
					...
					{
						if (projectile.type == ProjectileType<Solarang>() && projectile.owner == player.whoAmI)
						{
                            projectile.ai[0] = 2;
						}
					}
			...
			else
			{
				...
				{
					Projectile.NewProjectile(source, position, velocity, ProjectileType<Solarang>(), damage, knockback, player.whoAmI);
				}
			}
			return false;
		}
		...
		public override bool CanUseItem(Player player)
		{
			if (player.altFunctionUse == 2)
			{
				return player.ownedProjectileCounts[ProjectileType<Solarang>()] > 0;
			}
			else
			{
				return player.ownedProjectileCounts[ProjectileType<Solarang>()] <= 0;
    ...
    public class Solarang : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            else if (Projectile.ai[0] == 1f)
            {
                ...
                Vector2 acc = (target.velocity - Projectile.velocity) / 10 + (target.Center - Projectile.Center).RotatedBy(2 * Math.PI / 5) / 300;
                if (acc.Length() > 1)
                {
                    acc.Normalize();
                }
                ...
                attackTimer++;
                if (attackTimer % 60 < 20 && attackTimer % 2 == 0)
                {
                    if (attackTimer > 40) attackTimer = 0;
                    Vector2 shotDirection = target.Center - Projectile.Center;
                    shotDirection.Normalize();
                    shotDirection *= 8;
                    Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, shotDirection, 85, Projectile.damage, 0, Projectile.owner);

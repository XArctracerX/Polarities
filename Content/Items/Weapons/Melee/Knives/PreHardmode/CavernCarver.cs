...
namespace Polarities.Content.Items.Weapons.Melee.Knives.PreHardmode
{
    public class CavernCarver : ModItem
    {
        ...
        {
            CreateRecipe()
                .AddIngredient(ItemType<AlkalineFluid>(), 4)
                .AddIngredient(ItemType<Limestone>(), 20)
    ...
    {
        public override string Texture => "Polarities/Content/Items/Weapons/Melee/Knives/PreHardmode/CavernCarver";
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 4; i++)
            {
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustType<LimestoneDust>(), Projectile.velocity.X, Projectile.velocity.Y);
            }
        }
    }
}
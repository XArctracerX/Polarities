...
namespace Polarities
{
	public class PolaritiesSystem : ModSystem
	{
		...
		private static bool NonReplaceableTileForBiomes(Tile tile)
		{
			...
			return tileType == TileID.Granite || tileType == TileID.Marble 
				|| tileType == TileID.SnowBlock || tileType == TileID.IceBlock || tileType == TileID.BreakableIce 
				|| tileType == TileID.MushroomGrass || tileType == TileID.MushroomTrees 
				|| tileType == TileID.Sandstone || tileType == TileID.HardenedSand || wallType == WallID.Sandstone || wallType == WallID.HardenedSand
						...
						if (Main.rand.NextBool(3))
						{
							...
							while (i < holeSize)
							{
								...
								if (!holePoints.Contains(newPoint) && newPoint.X >= 0 && newPoint.X < Main.maxTilesX && newPoint.Y >= 0 && newPoint.Y < Main.maxTilesY)
								{
									...
									if (tile.TileType == limestoneTile) tile.HasTile = false;
		...
		{
			/*TODO: if (SLWorld.subworld) return;*/
